...
export default function Onboarding() {
    ...
    const toggleClass = (id) => {
        setData(
            'trademark_classes',
            data.trademark_classes.includes(id)
                ? data.trademark_classes.filter((c) => c !== id)
                : [...data.trademark_classes, id]
        );
    };

    const handleSearchClasses = async () => {
        if (!data.business_activity) return;
        setSearchingClasses(true);
        try {
            const res = await axios.post('/trademark/search-classes', {
                description: data.business_activity,
            });
            setMatchedClasses(res.data.classes); // [{id, name, description, recommended}]
        } catch (err) {
            setMatchedClasses([]);
        } finally {
    ...
    // ---------- Step 3 ----------
    const classCount = data.trademark_classes.length || 1;
    const plan = PLANS[selectedPlan];
    const totalPrice = plan.price * classCount;

    const choosePlanAndNext = () => {
                ...
                >
                    <div className="flex items-start gap-3">
                        <input
                            ...
                            name="trademarkType"
                            value={t.value}
                            checked={data.trademark_type === t.value}
                            onChange={() => setData('trademark_type', t.value)}
    ...
    <div>
        ...
        <div className="flex justify-end mb-6">
            <button
                ...
                disabled={!data.business_activity || searchingClasses}
                    ...
                    {matchedClasses.map((cls) => (
                        <label
                            ...
                            className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition ${
                                data.trademark_classes.includes(cls.id)
                                    ? 'border-blue-600 bg-blue-50'
                        ...
                        >
                            <input
                                type="checkbox"
                                checked={data.trademark_classes.includes(cls.id)}
        ...
        <div className="flex justify-between mt-6">
            <button
                onClick={back}
                className="flex items-center gap-1 border border-gray-300 rounded-lg px-6 py-2 text-gray-700 hover:bg-gray-50 transition"
            >
                ← Back
            </button>
            <button
                onClick={next}
                disabled={!data.business_activity}
                className="bg-blue-900 text-white px-6 py-2 rounded-lg hover:bg-blue-800 disabled:opacity-40 transition flex items-center gap-1"
            >
                Next →
            </button>
                ...
                {step === 3 && (
                    <div>
                            ...
                            {Object.entries(PLANS).map(([key, p]) => (
                                <label
                                    ...
                                    className={`block border rounded-lg p-4 cursor-pointer transition ${
                                        selectedPlan === key ? 'border-blue-600 bg-blue-50' : 'border-gray-200'
                                    }`}
                                >
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <select
                                disabled
                                className="border border-gray-300 rounded-lg px-4 py-2 bg-gray-100 text-gray-500"
                            >
                                <option>Class {data.trademark_classes[0] ?? 25}</option>
                            </select>
                            <div className="text-right">
                    ...
                    <div>
                        <div className="mb-6">
                            <span className="text-lg font-bold">
                                <span className="bg-yellow-400 px-1">VAKIL</span>SEARCH
                            </span>
                        </div>
                                ...
                                <div className="flex">
                                    <div className="w-1/3 border-r border-gray-200">
                                        <button
                                            ...
                                            className={`w-full text-left px-4 py-4 border-l-4 ${
                                                paymentMethod === 'upi'
                                                    ? 'border-blue-900 bg-white font-semibold'
                                    ...
                                    <div className="w-2/3 p-6">
                                            ...
                                            <>
                                                ...
                                                <div className="flex gap-4 mb-4">
                                                    {['Google pay', 'PhonePe', 'Paytm', 'Other UPI ID'].map((app) => (
                                                        ...
                                                        >
                                                            <div
                                                                className={`w-12 h-12 rounded-full border-2 flex items-center justify-center text-xs ${
                                                                    selectedUpiApp === app
                                                                        ? 'border-blue-900'
                                                                        : 'border-gray-200'
                                                                }`}
                                        ...
                                        {paymentMethod === 'card' && (
                                            <h3 className="font-semibold text-lg mb-4">Pay with Card</h3>
                                        )}

                                        {paymentMethod === 'netbanking' && (
                                            <h3 className="font-semibold text-lg mb-4">Pay with Net Banking</h3>
                                        )}
